...
function (Z, z.axes, z.axes.names = NULL, p, ax = NULL, ax.col, 
    ax.name.size, alpha = 0.5, pch.samples.size = 1, c.hull.n = 10, 
    class.vec, exp.factor = 1.2, label = TRUE, label.size, large.scale = FALSE, 
    line.width = 1, markers = TRUE, marker.size, max.num, means.plot = FALSE, 
    offset = rep(0.5, 4), pch.means = 15, pch.means.size = 1, 
    pos, predictions.mean = NULL, predictions.sample = NULL, 
    ort.lty = 1, specify.bags, specify.classes, strepie = c(1, 
        1), Title = NULL, Tukey.median = TRUE, Z.means.mat) 
{
    ...
    .axes.plot <- function(Z, Z.means.mat, z.axes, z.axes.names, 
        ...
        marker.size, offset, pos, strepie, predictions.sample, 
        predictions.mean, ort.lty) {
        if (is.null(ax)) 
            axes <- NULL
        else axes <- (1:p)[ax]
    ...
    if (means.plot == FALSE & large.scale == FALSE) {
        ...
        predictions <- .axes.plot(Z = Z, Z.means.mat = Z.means.mat, 
            ...
            marker.size = marker.size, offset = offset, pos = pos, 
            strepie = strepie, predictions.sample = predictions.sample, 
            predictions.mean = NULL, ort.lty = ort.lty)
    }
    if (!is.null(specify.classes)) 
