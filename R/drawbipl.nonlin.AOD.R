...
function (Z, z.axes, z.axes.names = NULL, p, ax = NULL, ax.col, 
    ax.name.col, ax.name.size, alpha = 0.5, pch.samples = 1, 
    pch.samples.size = 1, c.hull.n = 10, class.vec, exp.factor = 1.2, 
    label = TRUE, label.size, large.scale = FALSE, line.width = 1, 
    tra.width = 1, markers = TRUE, marker.size, max.num, means.plot = FALSE, 
    offset = rep(0.5, 4), parplotmar = rep(3, 4), pch.means = 15, 
    pch.means.size = 1, pos, predictions.mean = NULL, predictions.sample = NULL, 
    ort.lty = 1, specify.bags, specify.classes = 1, strepie = c(1, 
        1), Title = NULL, Tukey.median = TRUE, Z.means.mat = NULL, 
    ...
    xpd = FALSE, ...) 
{
    ...
    .axes.plot <- function(ax, z.axes, z.axes.names, predictions.sample, 
        predictions.mean, Z, Z.means.mat, p, ax.col, ax.name.col, 
        ax.name.size, markers, label, label.size, marker.size, 
        offset, pos, strepie, ort.lty) {
        if (is.null(ax)) 
            axes <- NULL
        else axes <- (1:p)[ax]
        predictions <- NULL
    ...
    .trajectories.plot <- function(z.axes, z.axes.names, p, ax, 
        ax.col, ax.name.col, pos, tra.width) {
        if (is.null(ax)) 
            axes <- NULL
        else axes <- (1:p)[ax]
        if (!is.element(pos[1], c("Hor", "Orthog", "Paral"))) 
        ...
        for (i in axes) {
            if (is.null(z.axes.names)) {
                ...
                axis.name <- z.axes.names[i]
                colr <- ax.name.col[i]
                colmark <- ax.col$tickmarker.col[i]
                colmarkval <- ax.col$marker.col[i]
            }
            lines(z.axes[[i]][, 1], z.axes[[i]][, 2], lwd = tra.width, 
                col = ax.col$ax.col[i])
    ...
    if (straight) 
        ...
        p = p, ax = ax, pos = pos, tra.width = line.width, ax.col = ax.col, 
        ax.name.col = ax.name.col)
    if (!is.null(CLPs)) 
        .CLP.plot(CLPs)
    if (!is.null(specify.classes)) 
