drawbipl.genbipl <-
function (Z, z.axes, z.axes.names = NULL, p, alpha = 0.5, ax = NULL, 
    ax.col, ax.name.size, exp.factor = 1.2, c.hull.n = 10, class.vec, 
    CLPs = NULL, CLPs.plot = CLPs.plot, colours = UBcolours, 
    label = TRUE, label.size, line.width = 1, markers = TRUE, 
    marker.size, max.num, offset = rep(0.5, 4), ort.lty = 1, 
    parplotmar = rep(2, 4), pch.means = 15, pch.means.size = 1, 
    pch.samples = 1, pch.samples.size = 1, pos, prediction.regions = FALSE, 
    predictions.mean = NULL, predictions.sample = NULL, specify.bags, 
    specify.classes = NULL, straight = TRUE, strepie = c(1, 1), 
    Title = NULL, Tukey.median = TRUE, Z.means.mat = NULL, zoomval = NULL, 
    x.grid = 0.5, y.grid = 0.5, plot.symbol = 16, plot.symbol.size = 0.5, 
    colours.pred.regions = 1:8) 
{
    ...
    .samples.plot <- function(Z, pch.samples.size, pch.samples, 
        ...
        for (j in classes) {
            ...
            Z.class <- data.frame(Z.class[, 1:2], pch.samples.samp = Z.class[1, 
                3], colr = (Z.class[1, 4]), lty = Z.class[1, 
    ...
    if (is.numeric(zoomval)) 
        plot(Z[, 1] * exp.factor, Z[, 2] * exp.factor, xlim = zoomval[1:2], 
            ylim = zoomval[3:4], xaxt = "n", yaxt = "n", xlab = "", 
    ...
    category.predictions.list <- NULL
    if (prediction.regions) {
        category.predictions.list <- vector("list", length(CLPs))
        for (j in 1:length(CLPs)) {
            prediction.mat <- CLPs[[j]][, ]
            category.vec <- rep(NA, nrow(Z))
            dev.new()
            par(pty = "s", mar = parplotmar)
            if (is.null(zoomval)) 
            ...
            if (is.numeric(zoomval)) 
                plot(Z[, 1] * exp.factor, Z[, 2] * exp.factor, 
                  xlim = zoomval[1:2], ylim = zoomval[3:4], xaxt = "n", 
                  yaxt = "n", xlab = "", ylab = "", type = "n", 
                  xaxs = "i", yaxs = "i", asp = 1)
            usr <- par("usr")
            pred.regions(mat = prediction.mat, x.grid = x.grid, 
                y.grid = y.grid, plot.symbol = plot.symbol, size = plot.symbol.size, 
                colours = colours.pred.regions)
            for (i in 1:nrow(Z)) {
                dist.vec <- apply(prediction.mat, 1, function(row, 
                  x, y) {
                  vec <- c(x, y, rep(0, ncol(prediction.mat) - 
                    2))
                  sqrt(sum((vec - row)^2))
                }, x = Z[i, 1], y = Z[i, 2])
                category.vec[i] <- (rownames(prediction.mat))[dist.vec == 
                  min(dist.vec)]
            }
            category.predictions.list[[j]] <- category.vec
        }
    }
    list(legend.lab.bags = legend.lab.bags, predictions = predictions, 
        usr = usr, category.predictions.list = category.predictions.list)
}
