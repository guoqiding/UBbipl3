...
function (Z, z.axes, z.axes.names = NULL, p, ax = NULL, ax.col, 
    ...
    ellipse.alpha, ellipse.kappa, exp.factor = 1.2, label = TRUE, 
    label.size, large.scale = FALSE, line.width = 1, markers = TRUE, 
    marker.size, max.num, means.plot = FALSE, offset = rep(0.5, 
        4), offset.m, pch.means = 15, pch.means.size = 1, pos, 
    pos.m, predictions.mean = NULL, predictions.sample = NULL, 
    ort.lty = 1, side.label, specify.bags, specify.beta.baskets, 
    specify.classes, specify.ellipses, strepie = c(1, 1), Title = NULL, 
    Tukey.median = TRUE, Z.means.mat, CVA.mean.dist, x.grid = 0.05, 
    y.grid = 0.05, plot.symbol = 15, plot.symbol.size = 0.25, 
    colours = 1:10) 
{
    ...
    if (large.scale) {
        ...
        xaxs = "i", yaxs = "i", asp = 1)
    usr <- par("usr")
    pred.regions(mat = CVA.mean.dist, x.grid = x.grid, y.grid = y.grid, 
        plot.symbol = plot.symbol, size = plot.symbol.size, colours = colours)
