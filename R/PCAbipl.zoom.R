...
function (X = Ocotea.data[, 3:8], G = NULL, X.new.samples = NULL, 
    ...
    density.plot = c(FALSE, "groups", "all"), exp.factor = 1.2, 
    ...
    ort.lty = 1, oblique.trans = NULL, orthog.transx = rep(0, 
        ...
        sum(c(ncol(X), ncol(X.new.vars)))), output = 1:9, parlegendmar = c(3, 
    ...
    text.width.mult = 1, Title = "", Titles.3d = c("", "", "x", 
        "y", "z"), Tukey.median = TRUE, zoomval = NULL) 
{
    ...
    if (dim.biplot == 1) {
        out.list <- return(PCAbipl.OneDim.zoom(X = X, G = G, 
            X.new.samples = X.new.samples, scaled.mat = scaled.mat, 
            ...
            quality.print = quality.print, offset = offset, reflect = reflect.mat, 
