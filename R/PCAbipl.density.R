...
function (X = Ocotea.data[, 3:8], G = NULL, X.new.samples = NULL, 
    ...
    colours.density = c("green", "yellow", "red"), colour.scheme = NULL, 
    colours = c(4:12, 3:1), columns = 1, correlation.biplot = FALSE, 
    ...
    label = TRUE, label.size = 0.6, large.scale = FALSE, layout.heights = c(100, 
        10), legend.type = c(means = FALSE, samples = TRUE, bags = FALSE), 
    line.length = c(1, 1), line.size = 2.5, line.type = 1:ncol(G), 
    line.width = 1, markers = TRUE, marker.size = 0.5, max.num = 2500, 
    means.plot = TRUE, n.int = rep(5, ncol(X)), offset = rep(0.5, 
        4), offset.m = rep(0.001, ncol(X)), oblique.trans = rep(0, 
        ncol(X)), orthog.transx = rep(0, ncol(X)), orthog.transy = rep(0, 
    ...
    select.origin = FALSE, side.label = rep("right", ncol(X)), 
    smooth.n = 100, specify.bags = NULL, specify.density.class = "allsamples", 
    specify.classes = dimnames(G)[[2]], specify.ellipses = NULL, 
    specify.beta.baskets = NULL, specify.xaxis = NULL, Title = NULL, 
    Tukey.median = TRUE) 
{
    X.new.vars <- NULL
