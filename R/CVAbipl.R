...
function (X = Ocotea.data[, 3:8], G = indmat(Ocotea.data[, 2]), 
    ...
    alpha.3d = 0.7, aspect.3d = "iso", ax.col.3d = "black", ax = 1:sum(c(ncol(X), 
        ...
        sum(c(ncol(X), ncol(X.new.vars)))), ax.name.size = 0.75, 
    ...
    Titles.3d = c("", "", "x", "y", "z"), Tukey.median = TRUE) 
{
    ...
    if (dim.biplot == 1) {
        out.list.1 <- CVAbipl.OneDim(X = X, G = G, X.new.samples = X.new.samples, 
            ...
            colours = colours, colours.means = colours.means, 
            colour.scheme = colour.scheme, columns = columns, 
            conf.alpha = conf.alpha, constant = constant, density.plot = density.plot, 
    ...
    if (dim.biplot == 3) {
        out.list.3 <- CVAbipl.3d(X = X, G = G, X.new.samples = X.new.samples, 
            ...
            col.text.3d = col.text.3d, factor.x = factor.x, factor.y = factor.y, 
            factor.3d.axes = factor.3d.axes, font.3d = font.3d, 
