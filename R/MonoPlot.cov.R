MonoPlot.cov <-
function (X = Flotation.data[, -1], scaled.mat = FALSE, as.axes = FALSE, 
    ...
    pos.m = rep(1, ncol(X)), samples.plot = FALSE, side.label = rep("left", 
        ncol(X)), line.length = c(1, 1), VJ.plot = FALSE, zoomval = NULL) 
{
    n <- nrow(X)
    p <- ncol(X)
    lam <- 1
    X.cent <- scale(X, center = TRUE, scale = FALSE)
    X.norm <- scale(X, center = TRUE, scale = TRUE)
    if (scaled.mat) 
        svd.out <- svd(X.norm)
    else svd.out <- svd(X.cent)
    Jmat <- diag(c(rep(1, dim.plot), rep(0, min(p, n) - dim.plot)))
    V <- svd.out$v
    U <- svd.out$u
    Sigma <- diag(svd.out$d)
    VSigma <- V %*% Sigma/sqrt(n - 1)
    if (lambda) {
        ...
        for (i in 1:p) {
            ...
            if (calibrate) {
                for (j in 1:length(marker.mat.invals[, 1])) Plot.marker(x = marker.mat.invals[j, 
                  ...
                  mark = marker.mat.invals[j, 3], expand = line.length[2], 
    ...
    list(cov.X = cov(X), cor.X = cor(X), adequacies = adequacies, 
        svd.out$d^2, svd.out$v, svd.out$v %*% diag(svd.out$d))
}
