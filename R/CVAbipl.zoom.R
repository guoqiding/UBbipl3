...
function (X = Ocotea.data[, 3:8], G = indmat(Ocotea.data[, 2]), 
    ...
    colours = c(1:8, 3:1), colours.means = c(1:8, 3:1), col.plane.3d = "lightgrey", 
    ...
    ID.3d = 1:nrow(X), label = TRUE, label.size = 0.6, large.scale = FALSE, 
    ...
    output = 1:14, parlegendmar = c(3, 1, 3, 1), parplotmar = rep(3, 
    ...
    zoomval = NULL) 
{
    ...
    if (!(dim.biplot == 1 || dim.biplot == 2)) 
        stop("Zoom only for 1D or 2D CVA biplots")
    dim.biplot <- dim.biplot[1]
    ...
    if (dim.biplot == 1) {
        out.list <- return(CVAbipl.OneDim.zoom(X = X, G = G, 
            X.new.samples = X.new.samples, e.vects = e.vects[1], 
            weightedCVA = weightedCVA, adequacies.print = adequacies.print, 
            ...
            conf.alpha = conf.alpha, columns = columns, constant = constant, 
            ...
            Tukey.median = Tukey.median, Title = Title, zoomval = zoomval))
    }
    if (dim.biplot == 2) {
        ...
        Z <- X.cent %*% Mmat[, e.vects[1:2]] %*% rotate.mat %*% 
            reflect.mat
        CVA.mean <- means.mat %*% Mmat[, e.vects[1:2]] %*% rotate.mat %*% 
            reflect.mat
        ...
        if (!is.null(zoomval) & !select.origin) {
            ...
            drawbipl.zoom(Z = Z, z.axes = z.axes, z.axes.names = c(dimnames(X)[[2]], 
                dimnames(X.new.vars)[[2]]), p = p, ax = ax, ax.col = ax.col, 
                ax.name.col = ax.name.col, ax.name.size = ax.name.size, 
                alpha = alpha, basket.beta = basket.beta, basket.n = basket.n, 
                c.hull.n = c.hull.n, class.vec = class.vec, conf.alpha = conf.alpha, 
                ...
                strepie = line.length, Title = Title, Tukey.median = Tukey.median, 
                Z.means.mat = Z.means.mat, zoomval = zoomval)
        }
        if (!is.null(oblique.trans) & ax.type == "interpolative") 
        ...
        if (select.origin) {
            ...
            if (!is.null(zoomval)) {
                ...
                zoomval <- zoom(zoomval)
                drawbipl.zoom(Z = Z, z.axes = z.axes, z.axes.names = c(dimnames(X)[[2]], 
                  dimnames(X.new.vars)[[2]]), p = p, ax = ax, 
                  ax.col = ax.col, ax.name.col = ax.name.col, 
        ...
        out.list <- list(SSP.B = SSP.B, SSP.W = SSP.W, Lmat = Lmat, 
            ...
            CVA.quality = CVA.quality, adequacy = adequacy.p, 
            predictions = uit$predictions)
    }
