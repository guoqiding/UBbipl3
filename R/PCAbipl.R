PCAbipl <- function (X = Ocotea.data[, 3:8], G = NULL, X.new.samples = NULL, 
    ...
    ort.lty = 1, oblique.trans = NULL, orthog.transx = rep(0, 
        ...
        sum(c(ncol(X), ncol(X.new.vars)))), output = 1:10, parlegendmar = c(3, 
    ...
    text.width.mult = 1, Title = "", Titles.3d = c("", "", "Dim 1", 
        "Dim 2", "Dim 3"), Tukey.median = TRUE, predictions.allsamples.onaxis = NULL, ...) 
{
    .doTrace({
    }, "on entry")
    {
        ...
        if (dim.biplot == 1) {
            out.list <- return(PCAbipl.OneDim(X = X, G = G, X.new.samples = X.new.samples, 
                scaled.mat = scaled.mat, e.vects = e.vects[1], 
        ...
        if (dim.biplot == 3) {
            out.list <- return(PCAbipl.3d(X = X, G = G, X.new = X.new.samples, 
                e.vects = e.vects[1:3], adjust.3d = adjust.3d, 
                alpha.3d = alpha.3d, ax.col = ax.col[[1]], aspect.3d = aspect.3d, 
                ax.col.3d = ax.col.3d, cex.3d = cex.3d, colour.scheme = colour.scheme, 
        ...
        if (dim.biplot == 2) {
            ...
            if (!is.null(specify.xaxis)) {
                if (!is.numeric(specify.xaxis)) 
                  ...
                  else specify.xaxis <- match(specify.xaxis, 
                    dimnames(X)[[2]])
            ...
            if (ax.type == "predictive") {
                if (is.null(X.new.vars)) 
                  ...
                  vnr.before.rotate.2 <- vnr.before.rotate[, 
                    e.vects[1:2]]
            ...
            uit <- drawbipl.bagalpha(Z = Z, z.axes = z.axes, 
                ...
                ax.name.size = ax.name.size, alpha = alpha, basket.n = basket.n, 
                basket.beta = basket.beta, c.hull.n = c.hull.n, 
                class.vec = class.vec, ellipse.alpha = ellipse.alpha, 
            ...
            if (select.origin) {
                ...
                dev.off()
                z.axes <- lapply(1:p, function(j, unscaled.X, 
                  means, sd, axes.rows, n.int, orthog.transx, 
                  orthog.transy) {
                  phi.vec <- diag(1/diag(axes.rows %*% t(axes.rows))) %*% 
                    axes.rows %*% c(orthog.transx[j], orthog.transy[j])
                  number.points <- 100
            ...
            if (any(legend.type)) {
                dev.new()
                blegend.colchar(quality.print = quality.print, 
                  QualityOfDisplay = fit.quality, classes = dimnames(G)[[2]], 
                  pch.means = as.vector(pch.means[1:J]), pch.samples = as.vector(pch.samples[1:J]), 
                  colours = as.vector(colours[1:J]), colours.means = colours.means, 
